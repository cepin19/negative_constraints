...
do
   ...
   echo -n $prob, >> results/para_beam_filter_subword.csv
	...
	python3 score_distance_suffix.py para_beam_filter_subword_$prob | tr "\n" "," >> results/para_beam_filter_subword.csv 

	bash score_notconstrained_constrained_suffix.sh  translated_para_beam_filter_subword_$prob >> results/para_beam_filter_subword.csv 
done
done
